...
def search( session ):
	...
	#Change Tab
	AllActive = BeautifulSoup(session.post( _core.ACTIVE_URL, data = payload, headers = _core.headers).content)
	print AllActive
	for x in payload:
		print x + "," + payload[x]
